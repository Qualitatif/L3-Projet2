...
let rec eval e env =
  ...
  | Let_anon(e1,e2) -> (p_verb 1 "let _ = ... in ...";eval e2 (eval_anon e1 env))
  | Ite(c,e1,e2) -> (p_verb 1 "if ... then ... else ...";if eval_cond c env then eval e1 env else eval e2 env)
  | PrInt(e) -> (p_verb 1 "affichage du résultat de l'expression";let v = eval e env in (match v with
  									   | VI(k) -> let _ = (print_int(k);print_string(";\n")) in v
  									   | VF(x,e) -> let _ = print_string("fonction...;\n") in v))
  | Semis(e1,e2) -> (p_verb 1 "separateurs";(eval e1 env ; eval e2 env))
  | Fun(x,e) -> (p_verb 1 "une fonction";VF (x,e))
  | App(e1,e2) -> (p_verb 1 "application d'une fonction ... ";(let v = eval e2 env in  
...
and eval_cond c env =
  match c with
  | True -> (p_verb 1 "c'est vrai!";true)
  | False -> (p_verb 1 "c'est faux!";false)
  | Le(e1,e2) -> (p_verb 1 "inférieur ou égal";(let v1 = (eval e1 env) in 
  				  let v2 = (eval e2 env) in
  				  match v1 with
  				  | VI(k1) -> (match v2 with
  				  			   | VI(k2) -> k1<=k2
  ...
  | Lt(e1,e2) -> (p_verb 1 "strictement inférieur";(let v1 = (eval e1 env) in 
  				  let v2 = (eval e2 env) in
  				  match v1 with
  				  | VI(k1) -> (match v2 with
  				  			   | VI(k2) -> k1<k2
  				  			   | VF(f,e) -> failwith "TypeError: Comaprison of int and function")
  				  | VF(f1,e) -> failwith "TypeError: Comparison of functions"))
  | Ge(e1,e2) -> (p_verb 1 "supérieur ou égal";(let v1 = (eval e1 env) in 
  				  let v2 = (eval e2 env) in
  				  match v1 with
  				  | VI(k1) -> (match v2 with
  				  			   | VI(k2) -> k1>=k2
  				  			   | VF(f,e) -> failwith "TypeError: Comaprison of int and function")
  				  | VF(f1,e) -> failwith "TypeError: Comparison of functions"))
  | Gt(e1,e2) -> (p_verb 1 "strictement supérieur";(let v1 = (eval e1 env) in 
  				  let v2 = (eval e2 env) in
  				  match v1 with
  				  | VI(k1) -> (match v2 with
  				  			   | VI(k2) -> k1>k2
  				  			   | VF(f,e) -> failwith "TypeError: Comaprison of int and function")
  				  | VF(f1,e) -> failwith "TypeError: Comparison of functions"))
  | Eq(e1,e2) -> (p_verb 1 "égal";(let v1 = (eval e1 env) in 
  				  let v2 = (eval e2 env) in
  				  match v1 with
  				  | VI(k1) -> (match v2 with
  				  			   | VI(k2) -> k1==k2
  				  			   | VF(f,e) -> failwith "TypeError: Comaprison of int and function")
  				  | VF(f1,e) -> failwith "TypeError: Comparison of functions"))
  | Neq(e1,e2) -> (p_verb 1 "différent";(let v1 = (eval e1 env) in 
  				  let v2 = (eval e2 env) in
  				  match v1 with
  				  | VI(k1) -> (match v2 with
  				  			   | VI(k2) -> k1<>k2
  				  			   | VF(f,e) -> failwith "TypeError: Comaprison of int and function")
  				  | VF(f1,e) -> failwith "TypeError: Comparison of functions"))
  | And(c1,c2) -> (p_verb 1 "ET";(eval_cond c1 env) && (eval_cond c2 env))
  | Or(c1,c2) -> (p_verb 1 "OU";(eval_cond c1 env) || (eval_cond c2 env))
  | Not(c) -> (p_verb 1 "NON";not (eval_cond c env))
